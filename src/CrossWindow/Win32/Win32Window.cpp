...
void Window::setSize(unsigned width, unsigned height)
{
    ...
    SetWindowPos(hwnd, nullptr, -1, -1, width + border.right + border.left,
                 height + border.top + border.bottom + titlebarHeight,
...
{
    RECT lpRect;
    DwmGetWindowAttribute(hwnd, DWMWA_EXTENDED_FRAME_BOUNDS, &lpRect,
                          sizeof(lpRect));
    int titlebarHeight =
        (GetSystemMetrics(SM_CYFRAME) + GetSystemMetrics(SM_CYCAPTION) +
         GetSystemMetrics(SM_CXPADDEDBORDER));
    return UVec2(lpRect.right - lpRect.left,
                 lpRect.bottom - lpRect.top - titlebarHeight);
}
